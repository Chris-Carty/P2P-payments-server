...
const paymentSettled = async (status, payment_id, event_id, settled_at) => {
  ...
  const initiateCommissionPayout = async (requestItems) => {
      ...
      accessToken.then(function(token) {
        ...
        const body = '{"beneficiary":{"type":"external_account","account_identifier":{"type":"iban","iban":"' + RvnuRecommenderIban + '"},"reference":"' + reference + '","account_holder_name":"' + RvnuRecommenderName + '"},"merchant_account_id":"' + rvnuMerchantAccountId + '","amount_in_minor":' + amountRounded + ',"currency":"GBP"}'
