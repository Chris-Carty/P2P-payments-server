          ...
          paymentInit.then(function(payment) {
            ...
            try {
              conn.query(query, (err, data) => {
                ...
                res.status(200).json(`https://payment.truelayer.com/payments#payment_id=${payment.id}&resource_token=${payment.resource_token}&return_uri=http://localhost:3000/status&c_primary=262626&c_secondary=000000&c_tertiary=000000`);
  ...
  try {
    conn.query(query, (err, data) => {
      ...
      if(data.length === 0) {
        ...
        res.status(408).send({ message: 'Invalid client_id or payment_request_id' })
        ...
        if (!isInThePast(new Date(paymentTimeout))) {
          ...
          try {
            conn.query(query, (err, data) => {
              ...
              return res.json({recommenderUsername, recommenderCommission, payerUsername,payerUsernameExpiry});
